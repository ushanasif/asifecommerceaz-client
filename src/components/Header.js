...
const Header = () => {
  ...
  return (
    <header className='h-16 shadow-md bg-white'>
      <div className='h-full container mx-auto flex flexi items-center px-4 justify-between'>
        <div>
          <Link to='/'>
            <Logo w={90} h={50} />
          </Link>
          ...
          <div className='relative group flex justify-center'>
            {
              user?._id && (
                  <div className='text-3xl cursor-pointer' onClick={()=>{setMenuDisplay(prev => !prev)}}>
                      {
                        user?.profilePic ? (<img src={user.profilePic} alt='Profile Pic' className='w-10 h-10 rounded-full' />):(<FaRegCircleUser />)
                      }
                  </div>
              )
            }
